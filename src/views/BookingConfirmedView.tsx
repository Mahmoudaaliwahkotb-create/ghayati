...
export interface BookingConfirmedInfo {
  parentName: string;
  studentName: string;
...
interface BookingConfirmedViewProps {
  ...
  bookingInfo?: BookingConfirmedInfo | null;
}
...
export const BookingConfirmedView: React.FC<BookingConfirmedViewProps> = ({
  ...
  const displayBookingId = bookingInfo?.bookingId || 'GH-84920';
  const displayStudent = bookingInfo?.studentName || 'أميرة أحمد محمد';
  const displayParent = bookingInfo?.parentName || 'أحمد محمد';
  const displayWhatsapp = bookingInfo?.whatsappNumber || '01012345678';
  const displayTeacher = selectedTeacher?.name || 'أ. محمود عليوة';
  const displayAvatar = selectedTeacher?.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=256';
  const displaySubject = selectedTeacher?.subject || 'اللغة العربية والتأسيس';
    ...
    <div className="w-full text-right">
      ...
      <Breadcrumbs
        ...
        items={[
          ...
          {
            label: `تأكيد الحجز ومتابعة الموعد (#${displayBookingId})`,
            isCurrent: true
          }
        ]}
      />
      ...
      <section className="mb-8 rounded-2xl bg-surface-container-lowest border border-secondary/30 shadow-sm overflow-hidden relative">
        ...
        <div className="p-6 md:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="flex items-start sm:items-center gap-5">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-1">
                <h1 className="text-headline-sm font-headline-sm text-on-surface font-bold">
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-surface-container text-primary font-label-md text-label-md font-semibold border border-primary-fixed-dim">
                  ...
                  <span>المرجع: #{displayBookingId}</span>
                </span>
              </div>
              <p className="text-body-md font-body-md text-on-surface-variant flex items-center gap-2">
                ...
                <span>
                  تم تسجيل الحصة بنجاح للطالب <strong>{displayStudent}</strong> مع المعلم <strong>{displayTeacher}</strong>.
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto self-end lg:self-center">
            <button
              onClick={() => alert(`تم إرسال بطاقة الموعد ورابط الحصة إلى رقم الواتساب: ${displayWhatsapp}`)}
        ...
        <div className="bg-surface-container-low/70 px-6 md:px-8 py-3 border-t border-outline-variant/40 flex flex-wrap items-center justify-between gap-3 text-body-sm font-body-sm text-on-surface-variant">
          <div className="flex items-center gap-2">
            ...
            <span>
              سيصلك رابط الانضمام المباشر على واتساب رقم <strong>{displayWhatsapp}</strong> قبل بدء الحصة بـ ١٥ دقيقة.
            </span>
          </div>
          <button
            onClick={() => setActiveTab('classes')}
      ...
      <section className="mb-8 p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant shadow-sm">
        <div className="flex items-center gap-2 text-primary font-bold mb-4">
          <span className="material-symbols-outlined text-xl">receipt_long</span>
          <h2 className="text-title font-title text-on-surface">ملخص بيانات الحجز المسجل في قاعدة البيانات</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-surface-container-low border border-surface-variant">
            <span className="text-label-sm text-on-surface-variant block mb-1">اسم ولي الأمر:</span>
          ...
          <div className="p-4 rounded-xl bg-surface-container-low border border-surface-variant">
            <span className="text-label-sm text-on-surface-variant block mb-1">اسم الطالب / الطفل:</span>
            <span className="text-body-md font-bold text-on-surface flex items-center gap-1.5">
          ...
          <div className="p-4 rounded-xl bg-surface-container-low border border-surface-variant">
            <span className="text-label-sm text-on-surface-variant block mb-1">رقم الواتساب المسجل:</span>
            <span className="text-body-md font-bold text-on-surface flex items-center gap-1.5 font-sans" dir="ltr">
          ...
          <div className="p-4 rounded-xl bg-surface-container-low border border-surface-variant">
            <span className="text-label-sm text-on-surface-variant block mb-1">المعلم المعتمد:</span>
            ...
            <div>
              ...
              <div className="bg-surface-container-low rounded-xl p-4 flex flex-wrap items-center justify-between gap-4 border border-outline-variant/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h5 className="text-title font-title text-on-surface font-bold">{displayTeacher}</h5>
                    ...
                    <p className="text-body-sm font-body-sm text-outline">{displaySubject} • معلم معتمد لدى منصة غايتي</p>
